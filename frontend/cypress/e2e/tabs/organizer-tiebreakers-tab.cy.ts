...
function stubTiebreakersTab(config: unknown = null) {
  ...
  cy.intercept('GET', '/api/organizer/tournament/tourney-1/standings-config', { statusCode: 200, body: config }).as('getConfig')
}
...
describe('Tiebreakers Tab', () => {
  ...
  it('saves config successfully after a change', () => {
    cy.intercept('PATCH', '/api/organizer/tournament/tourney-1/standings-config', { statusCode: 200, body: {} }).as('saveConfig')
    ...
    cy.request({ method: 'GET', url: '/api/organizer/tournament/tourney-1/standings-config', failOnStatusCode: false }).its('status').should('exist')
  })
})
