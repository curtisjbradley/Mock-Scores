...
export async function getScoreSheet(assignmentId: string, options?: { skipGuards?: boolean }): Promise<IScoreSheetFormat> {
    ...
    const tourney = (await dbQuery<{
        ...
        SELECT tf.case_name, tf.criminal_case, tf.p_witnesses_called,
    ...
    // Rows are (team_id, witness_id, position). If a team has set their call
    // order we use it; otherwise we fall back to the format default.
    ...
    // Build per-team ordered witness lists from call order rows
    ...
    // Resolve the witnesses each side calls, in order.
    // If a call order has been set use it; otherwise fall back to the format default.
    ...
    // Build a lookup keyed the same way AssignRoles reads/writes it:
    //   witness field  → `${fieldId}:${witnessId}`
    //   non-witness    → `${fieldId}` (no trailing colon)
    ...
    for (const cat of catRows) {
        ...
        if (!cat.witness_category) {
            ...
            // Merge P and D witness lists in the order they appear on the scoresheet:
            // all P-called witnesses first (in call order), then all D-called witnesses.
            // Dedup by witness id in case a swing witness appears in both lists.
