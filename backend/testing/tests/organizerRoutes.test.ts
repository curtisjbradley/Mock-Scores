...
describe('GET /api/organizer/tournament/', () => {
    it('returns 401 with no token', async () => {
        const res = await request(app).get('/organizer/tournament/');
    ...
    it('returns 200 with tournaments list', async () => {
        ...
        const res = await request(app).get('/organizer/tournament/').set(auth());
...
describe('POST /api/organizer/tournament/', () => {
    ...
    it('returns 201 on success', async () => {
        ...
        const res = await request(app).post('/organizer/tournament/').set(auth()).send(payload);
...
describe('DELETE /api/organizer/tournament/:tournamentId', () => {
    it('returns 400 for invalid UUID', async () => {
        const res = await request(app).delete('/organizer/tournament/not-a-uuid').set(auth());
    ...
    it('returns 204 on success', async () => {
        ...
        const res = await request(app).delete(`/organizer/tournament/${TOURNAMENT_ID}`).set(auth());
...
describe('GET /api/organizer/tournament/:tournamentId', () => {
    it('returns 403 when no tournament access', async () => {
        ...
        const res = await request(app).get(`/organizer/tournament/${TOURNAMENT_ID}`).set(auth());
        expect(res.status).toBe(403);
    });
...
describe('GET /api/organizer/tournament/:tournamentId/scorers', () => {
    it('returns 200 with scorers', async () => {
        ...
        const res = await request(app).get(`/organizer/tournament/${TOURNAMENT_ID}/scorers`).set(auth());
        expect(res.status).toBe(200);
...
describe('POST /api/organizer/tournament/:tournamentId/scorers', () => {
    ...
    it('returns 409 when scorer fields missing', async () => {
        mockAccess();
        const res = await request(app).post(`/organizer/tournament/${TOURNAMENT_ID}/scorers`).set(auth()).send({ email: 'a@b.com' });
    ...
    it('returns 200 on success', async () => {
        ...
        const res = await request(app).post(`/organizer/tournament/${TOURNAMENT_ID}/scorers`).set(auth()).send(scorer);
        expect(res.status).toBe(200);
    });
});
...
describe('DELETE /api/organizer/tournament/:tournamentId/scorers', () => {
    it('returns 400 when scorer_id missing', async () => {
        mockAccess();
        const res = await request(app).delete(`/organizer/tournament/${TOURNAMENT_ID}/scorers`).set(auth()).send({});
        expect(res.status).toBe(400);
    });
    ...
    it('returns 204 on success', async () => {
        ...
        const res = await request(app).delete(`/organizer/tournament/${TOURNAMENT_ID}/scorers`).set(auth()).send({ scorer_id: SCORER_ID });
        expect(res.status).toBe(204);
    });
});
...
describe('GET /api/organizer/tournament/:tournamentId/organizers', () => {
    it('returns 200 with organizers', async () => {
        ...
        const res = await request(app).get(`/organizer/tournament/${TOURNAMENT_ID}/organizers`).set(auth());
        expect(res.status).toBe(200);
    });
});
...
describe('POST /api/organizer/tournament/:tournamentId/organizers', () => {
    it('returns 400 when organizer fields missing', async () => {
        mockAccess();
        const res = await request(app).post(`/organizer/tournament/${TOURNAMENT_ID}/organizers`).set(auth()).send({ organizer: { name: 'Bob' } });
        expect(res.status).toBe(400);
    });
    ...
    it('returns 201 on success (new invite)', async () => {
        ...
        const res = await request(app)
            .post(`/organizer/tournament/${TOURNAMENT_ID}/organizers`)
            .set(auth())
...
describe('DELETE /api/organizer/tournament/:tournamentId/organizers', () => {
    it('returns 400 when id missing', async () => {
        mockAccess();
        const res = await request(app)
            .delete(`/organizer/tournament/${TOURNAMENT_ID}/organizers`)
            .set(auth())
            .send({ organizer: { name: 'Bob', email: 'b@c.com', role: 'delegate' } });
        expect(res.status).toBe(400);
    });
...
describe('GET /api/organizer/tournament/:tournamentId/courtrooms', () => {
    it('returns 200 with courtrooms', async () => {
        ...
        const res = await request(app).get(`/organizer/tournament/${TOURNAMENT_ID}/courtrooms`).set(auth());
        expect(res.status).toBe(200);
    });
});
...
describe('POST /api/organizer/tournament/:tournamentId/courtrooms', () => {
    it('returns 400 when name missing', async () => {
        mockAccess();
        const res = await request(app).post(`/organizer/tournament/${TOURNAMENT_ID}/courtrooms`).set(auth()).send({});
        expect(res.status).toBe(400);
    });
    ...
    it('returns 201 on success', async () => {
        ...
        const res = await request(app).post(`/organizer/tournament/${TOURNAMENT_ID}/courtrooms`).set(auth()).send({ id: 'c1', name: 'Room 1' });
        expect(res.status).toBe(201);
    });
});
...
describe('GET /api/organizer/tournament/:tournamentId/rounds', () => {
    it('returns 200 with rounds', async () => {
        mockAccess();
        mockDbQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 } as any);
        const res = await request(app).get(`/organizer/tournament/${TOURNAMENT_ID}/rounds`).set(auth());
        expect(res.status).toBe(200);
    });
});
...
describe('POST /api/organizer/tournament/:tournamentId/rounds', () => {
    it('returns 201 on success', async () => {
        ...
        const res = await request(app).post(`/organizer/tournament/${TOURNAMENT_ID}/rounds`).set(auth());
        expect(res.status).toBe(201);
    });
});
...
describe('GET /api/organizer/tournament/:tournamentId/teams', () => {
    it('returns 200 with teams', async () => {
        ...
        mockDbQuery
            ...
            .mockResolvedValueOnce({ rows: [], rowCount: 0 } as any); // invited
        const res = await request(app).get(`/organizer/tournament/${TOURNAMENT_ID}/teams`).set(auth());
        expect(res.status).toBe(200);
    });
});
...
describe('POST /api/organizer/tournament/:tournamentId/teams', () => {
    it('returns 400 when team fields missing', async () => {
        mockAccess();
        const res = await request(app).post(`/organizer/tournament/${TOURNAMENT_ID}/teams`).set(auth()).send({ team: { name: 'Eagles' } });
        expect(res.status).toBe(400);
    });
    ...
    it('returns 409 when team name already exists', async () => {
        ...
        const res = await request(app).post(`/organizer/tournament/${TOURNAMENT_ID}/teams`).set(auth()).send({ team: { name: 'Eagles', coach_email: 'c@d.com' } });
        expect(res.status).toBe(409);
    });
...
describe('GET /api/organizer/tournament/standings-templates', () => {
    it('returns 200 with templates', async () => {
        ...
        const res = await request(app).get('/organizer/tournament/standings-templates').set(auth());
        expect(res.status).toBe(200);
...
describe('PATCH /api/organizer/tournament/:tournamentId', () => {
    it('returns 400 when tournament body missing', async () => {
        mockAccess();
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}`).set(auth()).send({});
        expect(res.status).toBe(400);
    });
    ...
    it('returns 200 on success', async () => {
        mockAccess();
        mockDbQuery.mockResolvedValueOnce({ rows: [{ id: TOURNAMENT_ID }], rowCount: 1 } as any);
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}`).set(auth()).send({ tournament: { name: 'Updated', location: 'L', startDate: null, endDate: null } });
        expect(res.status).toBe(200);
    });
});
...
describe('GET /api/organizer/tournament/:tournamentId/format', () => {
    it('returns 200 with format', async () => {
        ...
        const res = await request(app).get(`/organizer/tournament/${TOURNAMENT_ID}/format`).set(auth());
        expect(res.status).toBe(200);
    });
...
describe('PATCH /api/organizer/tournament/:tournamentId/format', () => {
    it('returns 400 when witnesses called is negative', async () => {
        mockAccess();
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}/format`).set(auth()).send({ pWitnessesCalled: -1 });
        expect(res.status).toBe(400);
    });
    ...
    it('returns 200 on success', async () => {
        ...
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}/format`).set(auth()).send({ caseName: 'C', criminalCase: false });
        expect(res.status).toBe(200);
    });
});
...
describe('GET /api/organizer/tournament/:tournamentId/witnesses', () => {
    it('returns 200 with witnesses', async () => {
        ...
        const res = await request(app).get(`/organizer/tournament/${TOURNAMENT_ID}/witnesses`).set(auth());
        expect(res.status).toBe(200);
    });
...
describe('PATCH /api/organizer/tournament/:tournamentId/witnesses', () => {
    it('returns 400 when witness name is empty', async () => {
        mockAccess();
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}/witnesses`).set(auth()).send({ pWitnessNames: [''], dWitnessNames: [], swingWitnessNames: [] });
        expect(res.status).toBe(400);
    });
    ...
    it('returns 200 on success', async () => {
        ...
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}/witnesses`).set(auth()).send({ pWitnessNames: ['Alice'], dWitnessNames: ['Bob'], swingWitnessNames: [] });
        expect(res.status).toBe(200);
    });
});
...
describe('PATCH /api/organizer/tournament/:tournamentId/standings-config', () => {
    it('returns 400 when statsXml or standingsXml missing', async () => {
        mockAccess();
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}/standings-config`).set(auth()).send({ statsXml: '<x/>' });
        expect(res.status).toBe(400);
    });
    ...
    it('returns 200 on success', async () => {
        ...
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}/standings-config`).set(auth()).send({ statsXml: '<x/>', standingsXml: '<y/>' });
        expect(res.status).toBe(200);
    });
});
...
describe('PUT /api/organizer/tournament/:tournamentId/scorers', () => {
    ...
    it('returns 409 when scorer fields missing', async () => {
        mockAccess();
        const res = await request(app).put(`/organizer/tournament/${TOURNAMENT_ID}/scorers`).set(auth()).send({ email: 'a@b.com' });
        expect(res.status).toBe(409);
    });
...
describe('POST /api/organizer/tournament/duplicate/:tournamentId', () => {
    it('returns 400 for invalid UUID', async () => {
        const res = await request(app).post('/organizer/tournament/duplicate/not-a-uuid').set(auth());
        expect(res.status).toBe(400);
    });
    ...
    it('returns 201 on success', async () => {
        ...
        mockDbQuery
            ...
            .mockResolvedValueOnce({ rows: [], rowCount: 1 } as any);                // addTournamentOrganizer
        const res = await request(app).post(`/organizer/tournament/duplicate/${TOURNAMENT_ID}`).set(auth()).send({});
        expect(res.status).toBe(201);
    });
    ...
    it('returns 201 with scorers option', async () => {
        ...
        mockDbQuery
            ...
            .mockResolvedValueOnce({ rows: [], rowCount: 1 } as any);        // addTournamentOrganizer
        const res = await request(app).post(`/organizer/tournament/duplicate/${TOURNAMENT_ID}`).set(auth()).send({ scorers: true });
        expect(res.status).toBe(201);
    });
    ...
    it('returns 201 with courtrooms option', async () => {
        ...
        const res = await request(app).post(`/organizer/tournament/duplicate/${TOURNAMENT_ID}`).set(auth()).send({ courtrooms: true });
        expect(res.status).toBe(201);
    });
    ...
    it('returns 201 with witnesses option', async () => {
        ...
        mockDbQuery
            ...
            .mockResolvedValueOnce({ rows: [newT], rowCount: 1 } as any)
            .mockResolvedValueOnce({ rows: [], rowCount: 1 } as any);
        const res = await request(app).post(`/organizer/tournament/duplicate/${TOURNAMENT_ID}`).set(auth()).send({ witnesses: true });
        expect(res.status).toBe(201);
    });
});
...
describe('GET /api/organizer/tournament/:tournamentId/rounds/:round', () => {
    it('returns 400 for invalid round UUID', async () => {
        mockAccess();
        const res = await request(app).get(`/organizer/tournament/${TOURNAMENT_ID}/rounds/bad-uuid`).set(auth());
        expect(res.status).toBe(400);
    });
...
describe('PATCH /api/organizer/tournament/:tournamentId/format — witness validation', () => {
    it('returns 400 when pWitnessesCalled exceeds available', async () => {
        ...
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}/format`).set(auth()).send({ pWitnessesCalled: 5, dWitnessesCalled: null });
        expect(res.status).toBe(400);
    ...
    it('returns 400 when dWitnessesCalled exceeds available', async () => {
        ...
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}/format`).set(auth()).send({ pWitnessesCalled: null, dWitnessesCalled: 5 });
        expect(res.status).toBe(400);
...
describe('PATCH /api/organizer/tournament/:tournamentId/standings-config', () => {
    ...
    it('returns 200 on success', async () => {
        ...
        mockDbQuery
            ...
            .mockResolvedValueOnce({ rows: [], rowCount: 1 } as any);
        const res = await request(app).patch(`/organizer/tournament/${TOURNAMENT_ID}/standings-config`).set(auth()).send({ statsXml: '<s/>', standingsXml: '<st/>' });
        expect(res.status).toBe(200);
    });
...
describe('PUT /api/organizer/tournament/:tournamentId/organizers', () => {
    ...
    it('returns 400 when id missing', async () => {
        mockAccess();
        const res = await request(app).put(`/organizer/tournament/${TOURNAMENT_ID}/organizers`).set(auth()).send({ organizer: base });
        expect(res.status).toBe(400);
    });

    it('returns 400 when id is invalid uuid', async () => {
        mockAccess();
        const res = await request(app).put(`/organizer/tournament/${TOURNAMENT_ID}/organizers`).set(auth()).send({ organizer: { ...base, id: 'not-a-uuid' } });
        expect(res.status).toBe(400);
    });

    it('returns 409 when organizer already joined', async () => {
        mockAccess();
        const res = await request(app).put(`/organizer/tournament/${TOURNAMENT_ID}/organizers`).set(auth()).send({ organizer: { ...base, id: ORG_ID, has_joined: true } });
        expect(res.status).toBe(409);
    });
    ...
    it('returns 404 when organizer invite not found', async () => {
        mockAccess();
        mockDbQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 } as any);
        const res = await request(app).put(`/organizer/tournament/${TOURNAMENT_ID}/organizers`).set(auth()).send({ organizer: { ...base, id: ORG_ID, has_joined: false } });
        expect(res.status).toBe(404);
    });
...
describe('DELETE /api/organizer/tournament/:tournamentId/organizers — error cases', () => {
    ...
    it('returns 400 when id is invalid uuid', async () => {
        mockAccess();
        const res = await request(app).delete(`/organizer/tournament/${TOURNAMENT_ID}/organizers`).set(auth()).send({ organizer: { ...base, id: 'not-a-uuid' } });
        expect(res.status).toBe(400);
    });
    ...
    it('returns 404 when organizer not found', async () => {
        mockAccess();
        mockDbQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 } as any);
        const res = await request(app).delete(`/organizer/tournament/${TOURNAMENT_ID}/organizers`).set(auth()).send({ organizer: { ...base, id: ORG_ID, has_joined: false } });
        expect(res.status).toBe(404);
    });
});
...
describe('POST /api/organizer/tournament/:tournamentId/organizers — 500', () => {
    it('returns 500 when addOrganizer invite insert fails', async () => {
        ...
        const res = await request(app).post(`/organizer/tournament/${TOURNAMENT_ID}/organizers`).set(auth()).send({ organizer: { name: 'Bob', email: 'b@c.com', role: 'delegate' } });
        expect(res.status).toBe(500);
    });
});
...
describe('PUT /api/organizer/tournament/:tournamentId/courtrooms', () => {
    ...
    it('returns 200 on success', async () => {
        ...
        mockDbQuery.mockResolvedValueOnce({ rows: [row], rowCount: 1 } as any);
        const res = await request(app).put(`/organizer/tournament/${TOURNAMENT_ID}/courtrooms`).set(auth()).send({ id: 'c1', name: 'Updated' });
        expect(res.status).toBe(200);
    });
...
describe('PUT /api/organizer/tournament/:tournamentId/teams', () => {
    ...
    it('returns 400 when id missing', async () => {
        mockAccess();
        const res = await request(app).put(`/organizer/tournament/${TOURNAMENT_ID}/teams`).set(auth()).send({ team: base });
        expect(res.status).toBe(400);
    });

    it('returns 400 when id is invalid uuid', async () => {
        mockAccess();
        const res = await request(app).put(`/organizer/tournament/${TOURNAMENT_ID}/teams`).set(auth()).send({ team: { ...base, id: 'not-a-uuid' } });
        expect(res.status).toBe(400);
    });
...
describe('DELETE /api/organizer/tournament/:tournamentId/teams', () => {
    ...
    it('returns 400 when id is invalid uuid', async () => {
        mockAccess();
        const res = await request(app).delete(`/organizer/tournament/${TOURNAMENT_ID}/teams`).set(auth()).send({ id: 'not-a-uuid' });
        expect(res.status).toBe(400);
    });
